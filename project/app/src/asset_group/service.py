...
async def get_all_asset_groups(is_active: bool, is_archived: bool) -> List[AssetGroupOut]:
...
async def get_asset_group_by_id(asset_group_id: str) -> AssetGroupOut:
	asset_group = await AssetGroupDb.get(id=asset_group_id)
	return AssetGroupOut(**dict(asset_group))


async def update_asset_group(asset_group_id: str, asset_group: AssetGroupIn) -> AssetGroupOut:
...
async def delete_asset_group(asset_group_id: str) -> AssetGroupOut:
